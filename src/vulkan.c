...
static int create_image(VkFormat format, uint32_t width, uint32_t height,
			bool changeable, uint32_t mipLevels,
...
static int create_sampler(bool repeat, bool anisotropy, float mip_levels,
			  VkSampler *sampler)
{
	VkResult res;
...
static int create_depth_buffer(void)
{
	...
	if(create_image(vk.depth_format, vk.win_size.width, vk.win_size.height,
			false, 1, 1, VK_SAMPLE_COUNT_1_BIT,
...
extern int vk_create_texture(char *pth, struct vk_texture *texture)
{
	...
	if(vk_create_buffer(w*h*4, VK_BUFFER_USAGE_TRANSFER_SRC_BIT, true,
	                    &staging) < 0) {
	...
	if(create_image(VK_FORMAT_R8G8B8A8_SRGB, w, h, false,
	...
	if(create_sampler(true, true, texture->mip_levels, &texture->sampler)
			< 0) {
		return -1;
	}
...
extern int vk_create_skybox(char *pths[6], struct vk_texture *skybox)
{
	...
	if(vk_create_buffer(w*h*4*6, VK_BUFFER_USAGE_TRANSFER_SRC_BIT, true,
				&staging) < 0) {
		ERR_LOG(("Failed to create staging buffer"));
		return -1;
	}
	...
	if(create_image(VK_FORMAT_R8G8B8A8_SRGB, w, h, false, 1, 6,
	...
	if(create_sampler(false, false, 1.0f, &skybox->sampler) < 0) {
		return -1;
	}
